...
const VERDICT_STYLE: Record<Verdict, string> = {
  ...
  "Under Threshold": "bg-[rgba(138,180,248,0.1)] border-[#8ab4f8] text-[#8ab4f8]",
  "Not Serious": "bg-[rgba(75,85,99,0.1)] border-[#4b5563] text-[#9ca3af]",
  "Outside Window": "bg-[rgba(75,85,99,0.1)] border-[#4b5563] text-[#4b5563]",
  Unresolved: "bg-[rgba(75,85,99,0.1)] border-[#4b5563] text-[#9ca3af]",
};
...
function stateOf(p: Policy): { label: string; note: string; tone: string } {
  ...
  if (p.state === "closed") return { label: "Closed", note: "Window ended with no qualifying outage", tone: "text-[#9ca3af]" };
  if (today < p.window_start) {
    const n = daysBetween(today, p.window_start);
    return { label: "Pending", note: `Starts in ${n} ${n === 1 ? "day" : "days"}`, tone: "text-white" };
  }
  if (today < p.window_end) {
    const n = daysBetween(today, p.window_end);
    return { label: "Active", note: `Running, ${n} ${n === 1 ? "day" : "days"} left`, tone: "text-white" };
  }
...
export default function MyCover() {
  ...
  return (
    <div className="bg-[#090a0c] content-stretch flex flex-col items-start relative size-full" data-name="peril-my-cover">
      ...
      <div className="content-stretch flex gap-[32px] items-start p-[40px] relative shrink-0 w-full" data-name="my-cover-content">
        <div className="content-stretch flex flex-[1_0_0] flex-col gap-[24px] items-start min-w-px relative" data-name="portfolio-container">
          <div className="content-stretch flex items-end justify-between w-full gap-[16px]">
            ...
            {viewing && (
              <p className="font-mono text-[#9ca3af] text-[12px] m-0">
          ...
          {!viewing ? (
            <div className="peril-glass border border-solid flex flex-col gap-[12px] items-start p-[24px] rounded-[8px] w-full">
              ...
              <button
                ...
                className="bg-accent border-0 px-[16px] py-[8px] rounded-[6px] disabled:opacity-50"
              >
                <span className="font-mono font-bold text-[#090a0c] text-[13px]">{walletAvailable() ? "CONNECT WALLET" : "NO WALLET FOUND"}</span>
              </button>
            </div>
          ) : (
            <>
              <div className="[word-break:break-word] content-stretch flex gap-[16px] items-start leading-[normal] relative shrink-0 w-full whitespace-nowrap" data-name="metrics-row">
                {[
                  ...
                  <div key={m.label} className={`bg-[#121418] border ${m.accent ? "border-accent-line" : "border-[#1e222a]"} border-solid content-stretch flex flex-[1_0_0] flex-col gap-[8px] items-start min-w-px p-[20px] relative rounded-[8px]`} data-name={`m-card-${i}`}>
                    <p className="font-serif font-normal not-italic relative shrink-0 text-[#9ca3af] text-[12px] m-0">{m.label}</p>
                    <p className={`font-mono font-extrabold relative shrink-0 text-[24px] m-0 ${m.accent ? "text-accent-text" : "text-white"}`}>{m.value}</p>
                  </div>
                ))}
              </div>
              ...
              <div className="peril-glass border border-solid content-stretch flex flex-col items-start relative rounded-[12px] shrink-0 w-full" data-name="my-table-container">
                ...
                {policies.data && mine.length === 0 && (
                  <div className="border border-[#1e222a] border-solid p-[16px] w-full flex items-center justify-between">
                    <p className="font-mono text-[#9ca3af] text-[13px] m-0">This wallet holds no cover yet.</p>
                ...
                {mine.map((p) => {
                  ...
                  return (
                    <div key={p.policy_id} className={`border ${isSel ? "border-accent-line" : "border-[#1e222a]"} border-solid content-stretch flex items-center p-[16px] relative shrink-0 w-full`} data-name={`row-${p.policy_id}`}>
                      <p className="[word-break:break-word] flex-[1_0_0] font-serif font-semibold leading-[normal] min-w-px not-italic relative text-[13px] text-white m-0">
                        {serviceName(p.cover)}
                        <span className="block font-mono font-normal text-[#4b5563] text-[11px]">{p.policy_id}</span>
                      </p>
                      <p className="[word-break:break-word] flex-[1_0_0] font-mono font-normal leading-[normal] min-w-px relative text-[#9ca3af] text-[13px] m-0">
                        {p.window_start.slice(5)} - {p.window_end.slice(5)}
                      </p>
                      <p className="[word-break:break-word] flex-[1_0_0] font-mono font-normal leading-[normal] min-w-px relative text-[#9ca3af] text-[13px] m-0">{p.threshold_minutes} mins</p>
                      <p className="[word-break:break-word] flex-[1_0_0] font-mono font-normal leading-[normal] min-w-px relative text-[#9ca3af] text-[13px] m-0">{gen(p.premium)} GEN</p>
                      ...
                      <div className="[word-break:break-word] content-stretch flex flex-[1_0_0] flex-col font-serif font-normal gap-[4px] items-start leading-[normal] min-w-px not-italic relative" data-name="td-6">
                        ...
                        <p className="relative shrink-0 text-[#4b5563] text-[11px] m-0">{s.note}</p>
                      </div>
                      <div className="content-stretch flex items-start relative shrink-0 w-[140px]" data-name="td-7">
                        {p.state === "open" ? (
                          <button
                            ...
                            className={`${isSel ? "bg-accent border-[rgba(0,0,0,0)]" : "bg-[rgba(0,0,0,0)] border-accent-line"} border border-solid content-stretch flex flex-[1_0_0] items-start justify-center min-w-px px-[12px] py-[6px] relative rounded-[6px]`}
                          >
                            <span className={`[word-break:break-word] font-mono font-bold leading-[normal] relative shrink-0 text-[11px] whitespace-nowrap ${isSel ? "text-[#090a0c]" : "text-accent-text"}`}>
                              {isSel ? "CLAIMING" : "CLAIM PAYOUT"}
                            </span>
                          </button>
                        ) : (
                          <a href={`#/policy?id=${encodeURIComponent(p.policy_id)}`} className="bg-[rgba(0,0,0,0)] border border-[#1e222a] border-solid content-stretch flex flex-[1_0_0] items-start justify-center min-w-px px-[12px] py-[6px] relative rounded-[6px] no-underline">
                            <span className="[word-break:break-word] font-mono font-bold leading-[normal] relative shrink-0 text-[11px] text-white whitespace-nowrap">VIEW</span>
                          </a>
                        )}
                      </div>
                    </div>
              ...
              {refused && (
                <div className="[word-break:break-word] peril-glass border border-solid content-stretch flex flex-col gap-[12px] items-start p-[20px] relative rounded-[8px] shrink-0 w-full" data-name="rejection-panel">
                  ...
                  <div className="content-stretch flex font-mono font-normal items-start justify-between leading-[normal] relative shrink-0 text-[#9ca3af] text-[12px] w-full whitespace-nowrap" data-name="Frame">
                  ...
                  <p className="font-mono font-normal leading-[16px] min-w-full relative shrink-0 text-[#4b5563] text-[12px] w-[min-content] m-0">
        ...
        <div className="peril-glass border border-solid content-stretch flex flex-col gap-[20px] items-start p-[24px] relative rounded-[12px] shrink-0 w-[400px]" data-name="incident-picker-drawer">
          ...
          {!selected ? (
            <p className="font-serif text-[#9ca3af] text-[13px] leading-[18px] m-0">
              {viewing ? "No open cover to claim on. Paid and closed policies cannot be claimed again." : "Connect a wallet or add ?holder= to the link to pick a policy."}
            </p>
          ) : (
            <>
              <div className="content-stretch flex flex-col gap-[4px] items-start relative shrink-0 w-full" data-name="Frame">
                <p className="[word-break:break-word] font-serif font-normal leading-[normal] not-italic relative shrink-0 text-[#9ca3af] text-[11px] whitespace-nowrap m-0">SOURCE ENDPOINT</p>
                <div className="bg-[#14171f] border border-[#1e222a] border-solid content-stretch flex items-start p-[10px] relative rounded-[6px] shrink-0 w-full min-w-0">
                  <p className="[word-break:break-word] font-mono font-normal leading-[normal] overflow-hidden relative text-[#9ca3af] text-[11px] text-ellipsis whitespace-nowrap m-0">{`https://${selected.host}/api/v2/incidents.json`}</p>
                </div>
              </div>
              <div className="content-stretch flex flex-wrap gap-[6px] items-center justify-between relative shrink-0 w-full" data-name="Frame">
                <p className="[word-break:break-word] font-serif font-normal leading-[normal] not-italic relative text-[#9ca3af] text-[12px] m-0">
                ...
                <div className="content-stretch flex gap-[4px] items-center relative shrink-0" data-name="Frame">
                  <div className="relative shrink-0 size-[6px]">
                    <span className={`absolute block inset-0 max-w-none size-full rounded-full ${incidents.error ? "bg-[#ff3b30]" : incidents.list ? "bg-accent-line" : "bg-[#4b5563] peril-pulse"}`} />
                  </div>
                  <p className={`[word-break:break-word] font-mono font-normal leading-[normal] relative shrink-0 text-[11px] whitespace-nowrap m-0 ${incidents.error ? "text-[#ff3b30]" : "text-accent-text"}`}>
              ...
              <div className="content-stretch flex flex-col gap-[12px] items-start relative shrink-0 w-full" data-name="incident-list">
                ...
                {incidents.list && judged.length === 0 && (
                  <p className="font-serif text-[#9ca3af] text-[12px] leading-[17px] m-0">No incidents near this window in the provider&apos;s recent list.</p>
                )}
                {judged.map(({ inc, verdict, minutes }) => (
                  <div key={inc.id} className="bg-[#14171f] border border-[#1e222a] border-solid content-stretch flex flex-col gap-[8px] items-start p-[16px] relative rounded-[8px] shrink-0 w-full" data-name={`inc-${inc.id}`}>
                    <div className="content-stretch flex items-center justify-between relative shrink-0 w-full" data-name="Frame">
                      <a href={`https://${selected.host}/incidents/${inc.id}`} target="_blank" rel="noreferrer" className="font-mono font-normal leading-[normal] text-[#9ca3af] text-[11px] whitespace-nowrap no-underline hover:text-white">
                        {inc.id}
                      </a>
                    ...
                    <p className="[word-break:break-word] font-serif font-bold leading-[normal] not-italic relative shrink-0 text-[14px] text-white w-full m-0">{inc.name}</p>
                    <p className="[word-break:break-word] font-mono font-normal leading-[normal] relative shrink-0 text-[#4b5563] text-[11px] m-0">
                    ...
                    {verdict === "Eligible" && (
                      <button
                        ...
                        className="bg-accent border-0 content-stretch flex items-start justify-center py-[8px] relative rounded-[6px] shrink-0 w-full disabled:opacity-50"
                        data-name="settle-btn"
                      >
                        <span className="[word-break:break-word] font-mono font-extrabold leading-[normal] relative shrink-0 text-[#090a0c] text-[11px] whitespace-nowrap">
                          {busy === inc.id ? "CONFIRM IN YOUR WALLET…" : `Settle(${selected.policy_id}, ${inc.id})`}
                        </span>
                      </button>
                    )}
                  </div>
              ...
              <div className="flex flex-col gap-[8px] w-full border-t border-[#1e222a] pt-[16px]">
                <p className="font-serif text-[#9ca3af] text-[11px] m-0">OR PASTE AN INCIDENT ID</p>
                <div className="flex gap-[8px] w-full">
                  <input
                    value={manualId}
                    onChange={(e) => setManualId(e.target.value)}
                    placeholder="e.g. zkxwbgr0cnmx"
                    className="bg-[#14171f] border border-[#1e222a] border-solid rounded-[6px] px-[10px] py-[8px] font-mono text-[12px] text-white flex-1 min-w-0 outline-none focus:border-accent-line"
              ...
              {!walletAvailable() && (
                <p className="font-serif text-[#9ca3af] text-[12px] leading-[17px] m-0">A wallet is needed to send a claim. Anyone may settle; the payout only ever goes to the holder.</p>
              )}
